...
TerraLab uses **NVIDIA Isaac Sim** for simulation-based training and sim-to-real transfer. This document outlines the integration roadmap and milestones.
...
2. **Export to ROS2** — deploy trained policies to the TerraLab server and micro-ROS robots.
...
### 1.1 TerraLab scene in Isaac Sim
...
- [ ] **Topic naming** aligned with TerraLab server (`/robot_*/cmd`, `/robot_*/state`, `/terrain_map`).
- [ ] **Clock sync** (sim time vs real time) for replay and debugging.

**Deliverable:** Reproducible Isaac Sim scene that loads TerraLab bed + robots and connects to ROS2.
...
- [ ] **Terrain height map** topic published in same format as real TerraLab server.
...
- [ ] **Centralized vs decentralized** policies; compatibility with TerraLab server as coordinator.

### 3.3 Export and deployment

- [ ] **Policy export** to ONNX or TensorRT (or native Isaac format) runnable on Jetson.
- [ ] **ROS2 node** that loads policy, subscribes to TerraLab topics, publishes commands.
- [ ] **Safety**: velocity limits, workspace bounds, emergency stop — same as real robots.

**Deliverable:** Trained policies that run in ROS2 and can be tested on real TerraLab hardware.
...
- [ ] **Domain randomization** during training (already in Isaac): extend to TerraLab-specific params.
...
| TerraLab ROS2 stack | Planning, coordination, real robot interface |
...
- **Research:** Publishable results on embodied AI / terrain manipulation / sim-to-real with TerraLab.
- **Education:** Certification labs (Level 2–3) include “train in Isaac Sim, run on TerraLab” exercises.
- **Product:** TerraLab Research tier ships with an Isaac Sim scene and example policies.
...
- [TerraLab Design](TerraLab-Design.md) · [Architecture](architecture.md)
